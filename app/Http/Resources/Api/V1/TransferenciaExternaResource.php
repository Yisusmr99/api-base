<?php

namespace App\Http\Resources\Api\V1;

use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;

class TransferenciaExternaResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            'id'                  => $this->id,
            'tipo'                => $this->tipo?->value,
            'banco_externo'       => $this->banco_externo,
            'cuenta_externa'      => $this->cuenta_externa,
            'codigo_confirmacion' => $this->codigo_confirmacion,
            'estado'              => $this->estado,
            'fecha_envio'         => $this->fecha_envio?->toIso8601String(),
            'fecha_confirmacion'  => $this->fecha_confirmacion?->toIso8601String(),
            'transaccion'         => TransaccionResource::make($this->whenLoaded('transaccion')),
            'created_at'          => $this->created_at?->toIso8601String(),
            'updated_at'          => $this->updated_at?->toIso8601String(),
        ];
    }
}
